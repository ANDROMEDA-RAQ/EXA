...
def add_book_item(title, author, cover_path, col, row):
    ...
    try:
        ...
        # Resize images to be smaller to fit more books vertically
        img = img.resize(
            (150, 180), Image.LANCZOS
        )  # Reduced size for more vertical space
    ...
    info_frame.pack(side="left", padx=119)

    # Wrap title text
    words = title.split()
    lines = []
    current_line = []
    max_width = 20  # Reduced for smaller space
    ...
    # Adjusted placement of title
    title_label = tk.Label(
        info_frame,
        text="\n".join(lines),
        font=("Helvetica", 10, "bold"),  # Smaller font for space
        justify="left",
        wraplength=100,
    )
    title_label.pack(anchor="nw", pady=(5, 0))  # Adjust vertical padding here

    author_label = tk.Label(
        info_frame, text=f"by {author}", font=("Helvetica", 8)
    )  # Smaller font for author
    author_label.pack(anchor="nw", pady=(0, 5))  # Adjust vertical padding here
...
options_frame.configure(width=200, height=600)  # Adjusted height to match window size

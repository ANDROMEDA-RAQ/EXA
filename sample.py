...
try:
    logo_image = Image.open(r"C:\Users\JANREY\OneDrive\Desktop\pngkey.com-phillies-logo-png-528919.png")  # Replace with your logo image path
    ...
    right_logo_label.place(x=1535 - 90 - 10, y=5)  # Corrected to 800 - icon_width - padding
except FileNotFoundError:
    print("Error: Logo image not found. Please check the file path.")

# Adding text to the header
university_label = tk.Label(header, text="Polytechnic University of the Philippines", 
                           fg=style["text_color"], bg=style["bg_color"], 
                           font=("Arial", 16, "bold"))
university_label.place(relx=0.5, y=10, anchor="n")

library_label = tk.Label(header, text="LIBRARY", 
                        fg=style["text_color"], bg=style["bg_color"], 
                        font=("Arial", 14, "bold"))
...
try:
    ...
    my_books_icon = my_books_icon.resize((30, 30), Image.LANCZOS)  # Adjust size as needed
    ...
    readerboards_icon = readerboards_icon.resize((30, 30), Image.LANCZOS)  # Adjust size as needed
    readerboards_icon_photo = ImageTk.PhotoImage(readerboards_icon)

    # Home Button
    home_button = tk.Button(button_frame, image=home_icon_photo, command=lambda: print("Home button clicked"),
                            bg=style["button_color"], fg=style["button_text_color"], 
                            font=style["button_font"])
    home_button.image = home_icon_photo  # Keep a reference
    home_button.pack(side="left", padx=10, pady=5)

    # My Books Button
    my_books_button = tk.Button(button_frame, image=my_books_icon_photo, command=lambda: print("My Books button clicked"),
                                bg=style["button_color"], fg=style["button_text_color"], 
                                font=style["button_font"])
    ...
    # Readerboards Button
    readerboards_button = tk.Button(button_frame, image=readerboards_icon_photo, command=lambda: print("Readerboards button clicked"),
                                    bg=style["button_color"], fg=style["button_text_color"], 
                                    font=style["button_font"])
...
try:
    background_image = Image.open(r"C:\Users\JANREY\OneDrive\Desktop\output-onlinepngtools.png")  # Replace with your background image path
    background_image = background_image.resize((800, 450), Image.LANCZOS)  # Adjust the height to fit below button_frame
